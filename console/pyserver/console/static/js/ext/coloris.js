...
(function (window, document, Math) {
    ...
    for (var key in options) {
      switch (key) {
        ...
        case 'saveButton':
            ...
            if (options.saveButton.show) {
              display = 'block';
            }
  
            if (options.saveButton.label) {
              saveButton.innerHTML = options.saveButton.label;
            }
  ...
  function moveMarker(event) {
    ...
    var colorAreaDims = {
      width: colorArea.offsetWidth,
      height: colorArea.offsetHeight,
      x: picker.offsetLeft + colorArea.offsetLeft,
      y: picker.offsetTop + colorArea.offsetTop};
  ...
  function init() {
    ...
    settings.clearButton.label + "</button>") +
    
    ("<button id=\"clr-save\" class=\"clr-save\">" +
    settings.saveButton.label + "</button>") +
    ...
    addListener(saveButton, 'click', function (event) {
      pickColor();
      closePicker();
    });
  ...
  function addListener(context, type, selector, fn) {
    ...
    if (typeof selector === 'string') {
      ...
      fn = selector;
      context.addEventListener(type, fn, {passive:true});
    }
  }
