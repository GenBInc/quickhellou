...
func (c *client) removeSessionRefs(sessions []string) {
	...
	for _, m := range c.msgs {
		...
		log.Printf("Existing sessions: %s ", sessions)
		hasRef := c.stringInSlice(message.Body.SessionID, sessions)
		if message.Body.Name == "remoteClientRegistered" && !hasRef {
			log.Printf("Message \"%s\" of type {%s} has no session {%s} reference. Removing.", message.Body.Name, message.Body.Type, message.Body.SessionID)
		} else {
			tmp = append(tmp, m)
		}
		c.msgs = tmp
...
func (c *client) sendQueued(other *client) error {
	...
	log.Printf("++++++ client: client [%s] sending queued messages %v", c.id, len(c.msgs))
	for _, m := range c.msgs {
		sendServerMsg(other.rwc, m)
		log.Printf("++++++ client: client [%s] sent queued message {%s} from %s to %s", c.id, m, c.id, other.id)
	}
	c.msgs = nil
	return nil
}
