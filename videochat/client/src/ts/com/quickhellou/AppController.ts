...
export class AppController extends EventDispatcherService {
  ...
  private inlineLoadingParams: object
  ...
  constructor(roomId: string, initType: string, videoAppUrl: string) {
    super()
    this.loadingParams = new LoadingParams(roomId)
  ...
  public init(): void {
    ...
    formService
      .post(
        `${this.videoAppUrl}/api/video-settings/${this.loadingParams.roomID}`
      )
      .then((data) => {
        this.loadingParams.load(JSON.parse(data))
        this.onParamsLoaded()
      })
  }
    ...
    if (this.loadingParams.roomID) {
      ...
      if (StringUtils.equals(this.loadingParams.additionalParam, 'setup')) {
        ...
        confirmJoinButtonElement.addEventListener('click', (): void => {
          this.confirmJoin(confirmJoinDiv)
        })
  ...
  private roomSelectedHander(roomId: string): void {
    ...
    const roomLink: string = `${this.loadingParams.hostUrl}/r/${roomId}`
...
*/
   ...
   */
  private confirmJoin(confirmJoinDiv: HTMLElement): void {
    Log.log('confirmJoin')
    this.hide(confirmJoinDiv)
    this.setupCall()
  }
