...
class ClientSession:
  ...
  def remove_all_clients(self):
    ...
    # logging.info("[ClientSession::remove_all_clients] len: %d" % len(gc.get_referrers(self.clients[key])))
...
class ClientSessions:
  ...
  def add_initiator(self, client_id, session_id):
    logging.info('ClientSessions: add_initiator %s %s' % (client_id, session_id))
  ...
  def add_receiver(self, session_id, client_id, other_client_id, messages, is_forward):
    logging.info('ClientSessions: add_receiver %s for initiator %s' % (client_id, other_client_id))
  ...
  def get_clients(self):
    clients = []
    for key, session in self.sessions.items():
     clients = clients + session.clients.items()
    return clients
  ...
  def get_distinct_clients(self):
    ...
    logging.info('ClientSessions: distinct clients: %s ' % (distinct_clients))
  ...
  def remove_session(self, session_id):
    ...
    # logging.info('ClientSessions: Session %s removed' % session_id)
    logging.info('Removing session [%s] %d' % (session_id, len(self.sessions.items())))
  ...
  def get_session(self, session_id):
    ...
    logging.info('ClientSessions: Session %s doesn\'t exist' % session_id)
    return None
  ...
  def clear_sessions(self):
    for key in self.sessions.keys():
      ...
      logging.info('===========================> DELETE SESSION clear_sessions [%s]' % (key))
  ...
  def clear_duplicate_sessions(self):
    ...
    for session in ordered_sessions:
      ...
      # if client.get_id() in clients:
        logging.info('%%%% ClientSessions: removing client %s along with session %s' % (client.get_id(), session.id))
        session.remove_client(client.get_id())
        self.remove_session(session.id)
      else:
        clients.append(client.get_id())
  def __str__(self):
    return str(self.sessions.keys())
